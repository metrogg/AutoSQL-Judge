...
- **整体布局**：参考牛客网 SQL 练习界面，采用左题右码的两栏布局，中间有可拖拽的分隔条，支持调整左右宽度.
- **左侧面板**：
  - 顶部为「AI 出题对话」模块，学生可以用自然语言描述想练习的场景，由大模型生成相应 SQL 题目；
  - 中部为「当前题目」区域，展示题目标题、难度徽章、分值、题目来源（题库 / LLM）；
  - 底部为「数据库 Schema」与示例数据表格，按表维度展示字段列表与前若干行样例数据，帮助学生理解数据结构.
- **右侧面板**：
  - 顶部是一条编辑器工具栏，左侧显示「SQL 编辑器」标题，右侧以徽章形式展示当前数据集，例如 `MySQL · student_scores`；
  - 中部是基于 Monaco Editor 的 VS Code 风格深色 SQL 编辑器，支持行号、语法高亮和自动布局；
  - 编辑器下方是一条细长的工具条，包含「执行结果 / 自测输入 / 提交记录」三个 Tab，以及右侧的「运行」按钮；
  - 再往下是内容区域，根据当前 Tab 显示：
    - 执行结果：判题状态条（Pass/Fail/Error + 耗时）、查询结果预览表格、AI 讲解内容、标准答案 SQL；
    - 自测输入：用于记录自定义测试用例或期望输出的文本框；
    - 提交记录：当前会话内最近若干次提交的标题、时间与结果列表.
- **悬浮 AI 助教**：右下角提供一个可拖拽的悬浮对话框，围绕当前题目与最近一次判题结果进行多轮问答和知识讲解.
...
| V1.5.0 | 2025-12-13 | 更新学生端界面为牛客网风格左右分栏 + Monaco 编辑器 + 底部 Tab 运行条，并补充前端界面与交互设计章节. |
